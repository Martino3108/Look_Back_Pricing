...
The pricing methodology implemented in this project is based on a **continuous-time stochastic framework**, following the treatment of lookback options presented in Crépey (2013).
...
where \f$W_t\f$ is a standard Brownian motion.  
The associated running maximum process is defined as

$$ M_t = \sup_{0 \le s \le t} X_s.$$

A lookback option payoff is then written in the general form \f$\phi(X_T, M_T)\f$.

Although the model is formulated in **continuous time**, the Monte Carlo pricing procedure relies on an exact simulation of the joint law of the terminal value and the running maximum, \f$ (X_T, M_T) \f$, conditional on a continuous-time Euler approximation of \f$ X \f$.

More precisely, given a trajectory \f$ \hat X \f$ generated by the Euler scheme for \f$ X \f$, the maximum over each time interval \f$[t_i, t_{i+1}]\f$ is simulated exactly using the conditional distribution of the supremum of a Brownian motion with drift. This distribution is sampled using an auxiliary independent uniform random variable, as described in Proposition 6.9.2 of Crépey (2013).

This approach preserves the **continuous-time nature of the maximum process**, avoiding the discretization bias that would arise from a naive time-discretized maximum, while remaining compatible with a Monte Carlo framework.
