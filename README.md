...
where \f$ W_t \f$ is a standard Brownian motion. This dynamics corresponds to the explicit solution of the Black-Scholes model (applied to log-prices).
...
\f$ (X_T, M_T) \f$, is performed by sampling the conditional distribution of the maximum over each time interval using an auxiliary uniform random variable, as described in equation (6.20) of the reference.
