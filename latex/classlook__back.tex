...
Let $X_t$ denote the log-\/price process and $M_t = \sup_{0 \le s \le t} X_s$ its running maximum. Given an Euler discretization $\hat{X}$, the conditional law of the maximum on each interval is sampled using an independent uniform random variable, as stated in Paragraph 6.\+9.\+1.\+1 Black–\+Scholes Case.
...
The simulation of the pair $(X_T, M_T)$ follows the algorithm described in Stéphane Crépey (2013), Section 6.\+9, using:\+
\begin{DoxyItemize}
\item an Euler discretization for the underlying process, and
\item an exact conditional sampling of the running maximum on each time interval via an auxiliary uniform random variable.
\end{DoxyItemize}
